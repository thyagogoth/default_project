(function ($) {
    $.fn.orakuploader = function (options) {
        var settings = $.extend({
            ...
            orakuploader_miniaturas: "",
            orakuploader_url_admin: ""
        }, options);
        var holdername = this;
        ...
        $(document).on("change", "." + $(holdername).attr("id") + "Input", function () {
            if (settings.orakuploader_hide_in_progress == true) {
                if (parseInt(jQuery.data(holdername, 'currently_uploading')) == 1)
                    return false;
        ...
        $(holdername).on("click", ".picture_delete", function () {
            remove_img_bd(encodeURIComponent($(this).parent().attr('filename')), settings.orakuploader_url_admin);
    ...
    function orakuploaderHandle(files, holder, settings) {
        ...
        for (i = 0; i < files.length; i++)
        {
            if (jQuery.data(holder, "already_uploaded") > settings.orakuploader_maximum_uploads && (typeof settings.orakuploader_max_exceeded == 'function')) {
                if (msg_alert == false)
                    settings.orakuploader_max_exceeded();
                msg_alert = true;
                if (settings.orakuploader_hide_on_exceed == true)
    ...
    function upload(file, clone, place, holder, settings) {
        ...
        xhr.onreadystatechange = function ()
        {
            ...
            if (xhr.readyState == 4)
            {
                ...
                $(clone).html("<div class='picture_delete'>" + settings.orakuploader_file_delete_label + "</div>" + rotation_html + "<img src='" + settings.orakuploader_miniaturas + "/" + xhr.responseText + "' class='picture_uploaded'/> <input type='hidden' value='" + xhr.responseText + "' name='" + settings.orakuploader_field_name + "[]' />");
